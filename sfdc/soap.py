...
#
#    Usage: Create new instance of sfdcSoapClient, passing Salesforce API
#           credentials: sfClient = sfdcSoapClient(user, pass, token).
...
class Error(Exception):
    """Base class for sfdcSoapClient exceptions"""
    pass
...
class sfdcSoapClient:
    ...
    def update(self, sfObject, sfId,sfFields):    
        ...
        dom = xml.dom.minidom.parseString(self.response)
        if dom.getElementsByTagName('success'):            
            retSuccess = (dom.getElementsByTagName('success').item(0).firstChild.nodeValue == "true")            
        return retSuccess
    ...
    def parse_xml_node(node):        
        ret = None
        try:
            node_name = string.replace(node.nodeName,"sf:","")                                    
            if node.firstChild.nodeValue:                                                                        
                ret = { node_name: node.firstChild.nodeValue }
            else:                
                subnodes = {}                                                        
                for node2 in node.childNodes:                                            
                    if node2.firstChild:
                        subnodes.update(sfdcSoapClient.parse_xml_node(node2))                                     
                ret = { node_name: subnodes } 
        except Exception,e:
            ...
            for result in results:                
                ...
                for record in records:
                    ...
                    for node in record.childNodes:       
                        ret = sfdcSoapClient.parse_xml_node(node)
                        if ret:
                            recordFields.update(ret)                                                  
    ...
    #----------------------------------------------------------------
    #
    #    @param sfEmails: List  of Emails Dicts of API Field Name:Value 
    #
    #    @return: Set of dicts containing API Field Name: Value from
    #             the query
    #================================================================
    def email(self,sfEmails=[]):
        requestBody = '<soapenv:Body><urn:sendEmail>'
        
        for e in sfEmails:
            requestBody += '<urn:messages xsi:type="urn:SingleEmailMessage">'
            for sfFieldName, sfFieldValue in e.iteritems():
                requestBody += '<urn:%s>%s</urn:%s>' % (sfFieldName, sfFieldValue, sfFieldName)
            requestBody += '</urn:messages>'            
        
        requestBody += '</urn:sendEmail></soapenv:Body>'                         
